...
print("""<div class="container"><h1 class="text-center" >Citations sur le thème du voyage</h1></div>""")

print("""<div class="container">
		<img src="virus.png" class="mx-auto d-block" style="width:40%"> 
	</div>""")

conn = psycopg2.connect(database="bdd_garnaud", user=config.user, password=config.password, host='localhost')
cur = conn.cursor()
cur.execute("""SELECT * FROM blagues WHERE citations LIKE '%travel%' OR citations LIKE '%countries%' OR citations LIKE '%plane%' OR citations LIKE '%train%' OR citations LIKE '%visit%' OR citations LIKE '%hotel%' OR citations LIKE '%photos%';""")
